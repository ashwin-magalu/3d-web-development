...
walls.position.y = 2.5 / 2; // so walls sits on the floor, else it will be buried below the floor
house.add(walls);
...
roof.rotation.y = Math.PI * 0.25; // rotate by 45 degrees
roof.position.y = 2.5 + 1 / 2; // so roof sits on the walls
house.add(roof);
...
renderer.setSize(sizes.width, sizes.height);
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
renderer.setClearColor("#262837"); // Adding fog color to the renderer
